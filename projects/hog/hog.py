...
from ossaudiodev import SOUND_MIXER_NRDEVICES
...
def swine_align(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    gcd_flag = False
    for gcd in range(min(player_score,opponent_score),0,-1):
        if player_score % gcd == 0 and opponent_score % gcd == 0 and gcd >= 10:
            gcd_flag = True
            break
    
    if gcd_flag and opponent_score>= 10:
        return True
    else :
        return False
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while True:
        if who == 0:
            score_gain = take_turn(strategy0(score0,score1),score1,dice)
            score0 = score0 + score_gain
            ...
            score_gain = take_turn(strategy1(score1,score0),score0,dice)
            score1 = score1 + score_gain
            say = say(score0,score1)
            if score1 >= goal:
                break
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    "*** YOUR CODE HERE ***"
    def say(score0,score1):
        temp_high = running_high
        if who == 0:
            current_score = score0
        else:
            current_score = score1
        temp_sub = current_score - last_score
        if temp_sub > temp_high:
            temp_high = temp_sub
            print(temp_high,'point(s)!','The most yet for Player',who)
        return announce_highest(who,current_score,temp_high)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    best_score = 0
    while i <= 10:
        gain_score = make_averaged(roll_dice, trials_count)(i,dice)
